...
export class APIManager {
    ...
    async getAPIInfo(serviceName, includeExamples = true, includeCompatibility = true) {
        await this.initialize();
    ...
    async getSnippets(apiName, methodName, format = 'vscode') {
        await this.initialize();
        const snippets = [];
        for (const [serviceName, api] of this.apis) {
            // Skip if specific API requested and this isn't it
    ...
    generateCodeByStyle(serviceUri, methodName, parameters, style, includeErrorHandling) {
    ...
    generateCallbackCode(serviceUri, methodName, paramStr, includeErrorHandling) {
    ...
    generateAsyncCode(serviceUri, methodName, paramStr, includeErrorHandling) {
    ...
    generatePromiseCode(serviceUri, methodName, paramStr, includeErrorHandling) {
        const errorHandler = includeErrorHandling ? `
.catch(error => {
    console.log('Failed:', error.errorText);
    // Handle error
