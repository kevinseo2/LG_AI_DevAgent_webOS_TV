...
exports.WebOSChatParticipant = void 0;
const vscode = __importStar(require("vscode"));
class WebOSChatParticipant {
    ...
    async generateIntelligentLLMResponse(userQuery, context, stream, token) {
        try {
    ...
    async gatherMCPContext(userQuery, context) {
        const mcpContext = {
            available: false,
            apiList: [],
            relevantAPIs: [],
        ...
        try {
            ...
            if (mcpContext.relevantAPIs.length > 0) {
                console.log('🔍 Fetching detailed info for relevant APIs:', mcpContext.relevantAPIs.map((api) => api.serviceName));
    ...
    getDefaultMethodForAPI(apiName) {
    ...
    parseAPIListFromMarkdown(markdownText) {
        const apis = [];
    ...
    buildEnhancedWebOSExpertPrompt(contextInfo, mcpContext) {
...
${mcpContext.apiList.map((api) => `- **${api.serviceName}**: \`${api.serviceUri}\` (${api.category}) - ${api.description || 'No description'}`).join('\n')}` :
            '## ⚠️ API 정보를 MCP 서버에서 가져올 수 없습니다. 기본 정보를 사용합니다.';
        // 관련 API 정보
        const relevantApiInfo = mcpContext.relevantAPIs.length > 0 ?
            `## 🎯 사용자 질문과 관련된 API들
${mcpContext.relevantAPIs.map((api) => `- **${api.serviceName}**: \`${api.serviceUri}\` - ${api.description || 'No description'}`).join('\n')}` : '';
        // 특정 API 상세 정보
...
${mcpContext.specificAPIInfo}` : '';
        ...
        const codeExamplesInfo = mcpContext.codeExamples.length > 0 ?
            `## 💻 MCP 서버에서 제공하는 코드 예제
${mcpContext.codeExamples.map((example, index) => `### 예제 ${index + 1}\n\`\`\`javascript\n${example}\n\`\`\``).join('\n\n')}` : '';
...
사용자의 질문에 대해 위의 가이드라인을 따라 정확하고 실용적인 답변을 제공해주세요.`;
    ...
    async generateMCPBasedResponse(userQuery, context, stream, token) {
        try {
    ...
    async generateEnhancedResponse(userQuery, context, stream, token) {
        try {
            // 컨텍스트 정보 수집
            const contextInfo = await this.gatherContextInfo(context);
            // webOS TV API 정보 수집
    ...
    async generateContextualResponse(userQuery, contextInfo, apiInfo) {
    ...
    getProjectGreeting(projectType) {
        const greetings = {
        ...
        return greetings[projectType] || greetings.unknown;
    }
    async generateFallbackResponse(userQuery, context, stream, token) {
        // MCP 서버가 연결되어 있는지 확인
        if (!this.mcpClient || !this.mcpClient.isServerConnected()) {
            stream.markdown('⚠️ **MCP 서버에 연결할 수 없습니다.**\n\n' +
                'webOS TV API 정보를 가져오기 위해 MCP 서버가 필요합니다.\n' +
    ...
    async generateResponse(userQuery, context, token) {
        try {
            // 질문 유형 분석
    ...
    async handleAPIListQuery(queryType) {
        try {
            if (!this.mcpClient) {
    ...
    async handleAPIInfoQuery(queryType) {
        try {
            if (!this.mcpClient) {
                return '❌ MCP 서버에 연결할 수 없습니다.';
            }
    ...
    async handleCodeGenerationQuery(queryType) {
        try {
            if (!this.mcpClient) {
                return '❌ MCP 서버에 연결할 수 없습니다.';
            }
    ...
    async handleCodeReviewQuery(queryType) {
        return '## 🔍 코드 리뷰 기능\n\n' +
    ...
    async handleTroubleshootingQuery(queryType) {
        return '## 🛠️ 문제 해결 가이드\n\n' +
    ...
    async gatherContextInfo(context) {
        const contextInfo = {
            workspace: null,
            activeFile: null,
            projectType: 'unknown',
            ...
            if (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
                ...
                try {
                    ...
                    const relevantFiles = files
                        .filter(([name]) => name.includes('appinfo') ||
    ...
    findRelevantAPIs(userQuery, apis) {
        const lowerQuery = userQuery.toLowerCase();
        const relevantAPIs = [];
        const scoredAPIs = [];
    ...
    buildSystemPrompt(contextInfo, apiInfo) {
...
${apiInfo.available ?
            apiInfo.apis.map((api) => `- ${api.serviceName}: ${api.serviceUri}`).join('\n') :
            'API 정보를 가져올 수 없습니다. 기본 webOS TV API를 사용하세요.'}

## 관련 API 정보
${apiInfo.relevantAPIs.length > 0 ?
            apiInfo.relevantAPIs.map((api) => `- ${api.serviceName}: ${api.description}`).join('\n') :
            '관련 API를 찾을 수 없습니다.'}
...
사용자의 질문에 대해 전문적이고 도움이 되는 답변을 제공해주세요.`;
    ...
    async generateIntelligentAPIList(apiInfo, contextInfo) {
        ...
        if (apiInfo.available && apiInfo.apis.length > 0) {
            ...
            for (const [category, displayName] of Object.entries(categories)) {
                const categoryAPIs = apiInfo.apis.filter((api) => api.category === category);
    ...
    async generateIntelligentAPIInfo(userQuery, apiInfo, contextInfo) {
    ...
    async generateIntelligentCode(userQuery, apiInfo, contextInfo) {
        if (this.mcpClient && this.mcpClient.isServerConnected()) {
            try {
                const result = await this.mcpClient.callTool('webos_chat_assistant', {
                    userQuery: userQuery,
    ...
    async generateIntelligentCodeReview(userQuery, contextInfo) {
    ...
    async generateIntelligentTroubleshooting(userQuery, contextInfo) {
    ...
    async generateIntelligentGeneralResponse(userQuery, contextInfo, apiInfo) {
        return `## 🤖 webOS TV 개발 어시스턴트\n\n` +
    ...
    getProjectSpecificSuggestions(projectType) {
        const suggestions = {
        ...
        return suggestions[projectType] || suggestions.unknown;
    }
    getDefaultAPIs() {
        return [
            {
...
exports.WebOSChatParticipant = WebOSChatParticipant;
